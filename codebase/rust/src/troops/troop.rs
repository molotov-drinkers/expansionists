...
use crate::{
    ...
    player::{
        color::PlayerColor,
        player::{Player, PlayerStaticInfo},
    },
    root::root::RootScene,
    visual_debug,
};
use godot::{
    classes::{BoxMesh, CharacterBody3D, ICharacterBody3D, MeshInstance3D, StandardMaterial3D},
    prelude::*,
};

use super::{
    combat::{combat_stats::CombatStats, combat_stats::CombatTypes},
    speed::SpeedType,
...
impl ICharacterBody3D for Troop {
    fn init(base: Base<CharacterBody3D>) -> Troop {
        Troop {
            ...
            troop_activities: HashSet::from([TroopState::Idle, TroopState::Patrolling]),
...
impl Troop {
    ...
    pub fn set_orientation(&mut self, trajectory_vector: Vector3) {
        ...
        self.base_mut()
            .set_global_transform(Transform3D::new(basis, origin));
    }
    ...
    fn maybe_populate_trajectory_points(&mut self, virtual_planet: &Gd<VirtualPlanet>) {
            ...
            && self.troop_activities.contains(&TroopState::Patrolling)
        {
            ...
            let moving_to: Coordinates =
                virtual_planet.get_an_random_territory_coordinate(&self.deployed_to_territory);
        ...
        if self.moving_trajectory_is_set && !self.troop_activities.contains(&TroopState::Idle) {
            ...
            let current_target = {
                let Some(current_target) = self
                    .moving_trajectory_points
                    .get(self.current_trajectory_point)
                else {
            ...
            let on_the_last_waypoint =
                self.current_trajectory_point == (self.moving_trajectory_points.len() - 1);
    ...
    fn have_future_invasion_in_the_trajectory(&mut self) -> bool {
        ...
        if self.troop_activities.contains(&TroopState::Patrolling)
            && (self.current_trajectory_point + buffer_checker)
                < self.moving_trajectory_points.len() - 1
        {
            let check_future_invasion =
                self.moving_trajectory_points[self.current_trajectory_point + buffer_checker];
            let mut world = self.base().get_world_3d().expect("World to exist");
            let surface_point =
                SurfacePoint::get_surface_point(check_future_invasion, &mut world, None)
                    .expect("Expected to get surface point to check future invasion");
            if surface_point
                .bind()
                .get_surface_point_metadata()
                .territory_id
                .clone()
    ...
    pub fn highlight_trajectory(&mut self, trajectory: &Vec<Vector3>) {
        ...
        let mut highlighted_trajectories = self
            .base_mut()
            .get_parent()
