use crate::{
    globe::coordinates_system::{
        coordinates_system::CoordinatesSystem, virtual_planet::VirtualPlanet,
    },
    troops::{
        combat::combat_stats::CombatStats,
        surface::surface::Surface,
        troop::{Troop, TroopId, TroopState},
    },
    visual_debug,
};
use godot::prelude::*;
...
impl Troop {
    pub fn keep_fighting_if_combatting(&mut self, delta: f64, virtual_planet: &Gd<VirtualPlanet>) {
        ...
        if self
            .troop_activities
            .contains(&TroopState::Combating(CombatTypes::Attacking))
        {
            self.handle_attacking_behavior(delta, virtual_planet);
        } else if self
            .troop_activities
            .contains(&TroopState::Combating(CombatTypes::Defending))
        {
    ...
    fn find_closest_enemy_troop_to_be_attacked(
      ...
      .unwrap_or_else(|| panic!("Expected to find territory {touching_territory_id}, at engage_combat_if_needed"));

        let Some(enemy_player_id) = territory
        ...
        let closest_enemy_troop = enemy_troops
            ...
            .fold(
                None,
                |current_closest: Option<(Gd<Troop>, f32)>, enemy_troop_id| {
                    if let Some(enemy_troop) = Self::get_troop_by_id(virtual_planet, enemy_troop_id)
                    {
                        ...
                        let cannon_range_plus_buffer =
                            self.combat_stats.cannon.range + (self.combat_stats.cannon.range / 8.);

                        if close_to_the_cannon_range
                            && new_comparable_distance > cannon_range_plus_buffer
                        {
                        ...
                        match current_closest {
                            ...
                            Some((_, current_min_distance))
                                if new_comparable_distance >= current_min_distance =>
                            {
                                current_closest
                            }
    ...
    fn get_projectile_spawner_position(&self) -> Transform3D {
        ...
        let projectile_spawner = self.base().get_node_as::<Node3D>(path);

        projectile_spawner.get_global_transform()
    }

    fn set_trajectory_to_get_closer_to_enemy(
        &mut self,
    ...
    fn get_troop_by_id(
        ...
        let Some(troop) = root.try_get_node_as::<Troop>(&format!("troops/{troop_id}")) else {
    ...
    fn open_fire_on_the_enemy(
        &mut self,
        enemy_troop: Gd<Troop>,
        virtual_planet: &GdRef<'_, VirtualPlanet>,
    ) {
        let root = virtual_planet.get_root_from_virtual_planet();
        ...
        let mut projectiles_node = root.get_node_as::<Node3D>("troops/projectiles");
    ...
    fn die(&mut self) {
        ...
        self.get_virtual_planet_from_troop_scope()
            ...
            .inform_territory_departure(&self.base().get_name().to_string(), self.owner.player_id);
