...
impl Troop {
    pub fn trigger_combat_engage_if_needed(&mut self, virtual_planet: &Gd<VirtualPlanet>) {
      ...
      .unwrap_or_else(|| panic!("Expected to find territory {touching_territory_id}, at engage_combat_if_needed"));
        ...
        if territory.has_troops_from_different_players
            && self.is_the_territory_deployed_to(touching_territory_id)
        {
            ...
            if territory.current_ruler.is_none() {
                self.troop_activities.insert(TroopState::Combating(
                ...
                territory.current_ruler.as_ref().map(|ruler_static_info| {
                    if ruler_static_info.player_id != self.owner.player_id {
                        self.troop_activities
                            .insert(TroopState::Combating(CombatTypes::Attacking));
                    } else {
                        self.troop_activities
                            .insert(TroopState::Combating(CombatTypes::Defending));
    ...
    pub fn troop_is_combatting(&self) -> bool {
        for combat_type in CombatTypes::iter() {
            if self
                .troop_activities
                .contains(&TroopState::Combating(combat_type))
            {
                return true;
            }
        }
    ...
    fn remove_combatting_states(&mut self) {
        ...
        for combat_type in CombatTypes::iter() {
            self.troop_activities
                .remove(&TroopState::Combating(combat_type));
        }
    }
