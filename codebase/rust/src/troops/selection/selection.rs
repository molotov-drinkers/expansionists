...
impl Troop {
    pub fn set_order_to_move_to(&mut self, destination: Vector3, dest_territory_id: &TerritoryId) {
        ...
        let virtual_planet = &mut virtual_planet.bind_mut();

        let origin_territory =
            self.get_territory(self.deployed_to_territory.clone(), virtual_planet);
        ...
        let destination_territory =
            self.get_territory(self.deployed_to_territory.clone(), virtual_planet);
        destination_territory
            .add_territory_deployment(&self.base().get_name().to_string(), self.owner.player_id);
    ...
    fn get_territory<'a>(
        ...
        virtual_planet: &'a mut GdMut<'_, VirtualPlanet>,
    ) -> &'a mut Territory {
