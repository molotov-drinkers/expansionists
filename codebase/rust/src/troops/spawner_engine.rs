use crate::{
    globe::territories::territory::Territory,
    player::{
        color::PlayerColor,
...
use godot::{
    classes::{MeshInstance3D, Sprite3D, StandardMaterial3D},
    prelude::*,
};

use super::{mesh_map::TroopMesh, troop::Troop};

pub fn spawn_troop(root_scene: &mut RootScene, player: &mut Gd<Player>, territory: &mut Territory) {
    ...
    let mut sea_node = new_troop.find_child("sea").expect("Expected sea to exist");
    ...
    land_node.get_node_as::<Sprite3D>("selected").set_modulate(
        PlayerColor::get_troop_selected_color(&player_static_info.color),
    );
    sea_node.get_node_as::<Sprite3D>("selected").set_modulate(
        PlayerColor::get_troop_selected_color(&player_static_info.color),
    );
    ...
    new_troop.emit_signal(Troop::EVENT_TROOP_SPAWNED, &[]);
...
fn get_colored_troop_scenes(player: &PlayerStaticInfo) -> (Gd<Node3D>, Gd<Node3D>) {
    ...
    let land_troop = lands
        .get(&troop_meshes.land)
        .unwrap_or_else(|| panic!("Expected {:?} land mesh to exist", &troop_meshes.land));
    let sea_troop = seas
        .get(&troop_meshes.sea)
        .unwrap_or_else(|| panic!("Expected {:?} sea mesh to exist", &troop_meshes.sea));

    let land_scene_name = &land_troop.scene_name;
    let sea_scene_name = &sea_troop.scene_name;

    let land_mesh: Gd<PackedScene> =
        load(&format!("res://scenes/troops/land/{land_scene_name}.tscn"));
