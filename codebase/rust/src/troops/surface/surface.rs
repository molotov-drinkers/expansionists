...
impl Troop {
    ...
    pub fn set_surface_troop(&mut self) {
        let surface_point = SurfacePoint::get_troop_surface_point(self);
        ...
        } else if !surface_point.is_in_group(&Surface::Land.to_string())
    ...
    fn get_sea_and_land_mesh(&self) -> (Gd<Node3D>, Gd<Node3D>) {
        let sea_mesh = self.base().get_node_as::<Node3D>("sea");

        let land_mesh = self.base().get_node_as::<Node3D>("land");
