use crate::{
    globe::{
        coordinates_system::virtual_planet::VirtualPlanet, territories::territory::TerritoryState,
    },
    ...
    pub fn get_deployment_next_action(&mut self, virtual_planet: &mut Gd<VirtualPlanet>) {
        if self.waiting_for_deployment_following_action {
            ...
            if touching_territory_id == &self.deployed_to_territory {
                ...
                let territory_current_ruler = territory.current_ruler.as_ref();

                if territory
                    .territory_states
                    .contains(&TerritoryState::Unoccupied)
                    && !territory.has_troops_from_different_players
                {
                    territory
                        .territory_states
                        .insert(TerritoryState::OccupationInProgress);
                    territory
                        .territory_states
                        .remove(&TerritoryState::UnoccupiedUnderConflict);

                    let root = self.get_root_from_troop();
                    let player = Player::get_player_by_id(root, self.owner.player_id);
                    let player_static_info = player.bind().static_info.clone();
                    territory.player_trying_to_conquer = Some(player_static_info);
                } else if territory_current_ruler.is_some_and(|ruler_static_info| {
                    ruler_static_info.player_id == self.owner.player_id
                }) {
                    ...
                    territory
                        .territory_states
                        .insert(TerritoryState::OccupiedUnderConflict);
                    ...
                    && territory.has_troops_from_different_players
                {
                    territory
                        .territory_states
                        .insert(TerritoryState::UnoccupiedUnderConflict);
                    territory.player_trying_to_conquer = Some(self.owner.clone());
                    ...
                    godot_error!(
                        "Troop has no idea what to do after the deployment! ::: {}",
                        touching_territory_id
                    );
                }
            };
        }
