use fastrand;
use godot::{
    classes::{BoxMesh, BoxShape3D, CollisionShape3D, MeshInstance3D, StandardMaterial3D},
    prelude::*,
};
...
use crate::{
    globe::territories::{
        land::Land,
        territory::{Territories, Territory, TerritoryId, TerritoryState},
    },
    player::{color::PlayerColor, player::Player},
    root::root::RootScene,
    troops::{spawner_engine::spawn_troop, surface::surface::Surface},
};
...
impl VirtualPlanet {
    ...
    #[inline]
    pub fn get_planet_radius() -> f64 {
        1.0795 * 3.0
    }
    #[inline]
    pub fn get_num_of_latitudes() -> i16 {
        (90. * 2.5) as i16
    }
    #[inline]
    pub fn get_num_of_longitudes() -> i16 {
        (180. * 2.5) as i16
    }
    #[inline]
    pub fn get_surface_mesh_and_collider_size() -> Vector3 {
        Vector3::new(0.07, 0.07, 0.08)
    }
    ...
    pub fn create_virtual_sphere(&mut self) {
        for surface_point_metadata in self.surface_points_metadata.clone() {
            let surface_point = VirtualPlanet::create_surface_point_area(surface_point_metadata);
            self.base_mut().add_child(&surface_point);
        }
    }

    pub fn create_surface_point_area(
        ...
        surface_point
            .bind_mut()
            .set_surface_point_metadata(surface_point_metadata);
        surface_point
    }
    ...
    pub fn create_surface_collider(
    ...
    pub fn match_surface_points_and_territories(&mut self) {
        if !self.has_surface_points_matching_started {
            for surface_point_node in self.base().get_children().iter_shared() {
                ...
                if !bodies_overlapping_with_surface_point.is_empty() {
                    self.has_surface_points_matching_started = true;

                    for body_overlapping_with_surface_point in
                        bodies_overlapping_with_surface_point.iter_shared()
                    {
                        if let Ok(collided_land) =
                            body_overlapping_with_surface_point.try_cast::<Land>()
                        {
                            ...
                            if let Some(overlapped_territory) =
                                self.territories.get_mut(&territory_id)
                            {
                                ...
                                let surface_point_metadata =
                                    surface_point_bind.get_surface_point_metadata_mut();

                                overlapped_territory
                                    .coordinates
                                    .push(surface_point_metadata.lat_long);

                                let coordinates: Coordinates =
                                    VirtualPlanet::get_spawner_territory_coordinate(
                                        overlapped_territory,
                                    );
                                let cartesian = self
                                    .coordinate_map
                                ...
                                let mut dic_coordinates_map =
                                    dic_coordinates_map.to::<Dictionary>();

                                let mut coord_meta = Dictionary::new();
                                let _ = coord_meta.insert("territory_id", territory_id.clone());
                                let _ = coord_meta
                                    .insert("cartesian", surface_point_metadata.cartesian);
    ...
    pub fn get_an_random_territory_coordinate(&self, territory_id: &str) -> Coordinates {
        let territory = self
            .territories
            .get(territory_id)
            .expect("Expected territory to exist");
        let territory_coordinates = &territory.coordinates;
        if territory_coordinates.is_empty() {
    ...
    pub fn get_spawner_territory_coordinate(territory: &Territory) -> Coordinates {
        ...
        let territory_point = territory_coordinates.len() / 4;
    ...
    pub fn set_new_territory_ruler(territory: &mut Territory, player: &mut Gd<Player>) {
        ...
        territory
            .territory_states
            .remove(&TerritoryState::Unoccupied);
        territory
            .territory_states
            .remove(&TerritoryState::OccupationInProgress);
        territory
            .territory_states
            .remove(&TerritoryState::OccupiedUnderConflict);
        territory
            .territory_states
            .remove(&TerritoryState::UnoccupiedUnderConflict);
    ...
    fn spawner_troop_engine_checker(&mut self, delta: f64) {
        ...
        for (_territory_id, territory) in territories_with_rulers {
            ...
            if player.bind().troops_counter < player.bind().max_troop_allowed
                && (territory.all_troops_deployed_and_arrived.len() as u32)
                    < territory.organic_max_troops
            {
                ...
                if territory.next_troop_progress >= 100. {
                    ...
                    spawn_troop(&mut root_scene, &mut player, territory);
                } else {
                    // Should represent how many seconds should take for a troop to be spawned at the territory
                    territory.next_troop_progress = 100.
                        * territory.valid_seconds_elasped_since_last_troop
                        / territory.seconds_to_spawn_troop;
                }
            }
        }
    }
    ...
    pub fn get_mut_territory_from_virtual_planet(
        &mut self,
        territory_id: &TerritoryId,
    ) -> &mut Territory {
        self.territories.get_mut(territory_id).unwrap_or_else(|| {
            panic!(
                "Expected territory {territory_id} to exist: {:?}",
                territory_id
            )
        })
    }
    ...
    pub fn get_territory_from_virtual_planet(&self, territory_id: &TerritoryId) -> &Territory {
        self.territories.get(territory_id).unwrap_or_else(|| {
            panic!(
                "Expected territory {territory_id} to exist: {:?}",
                territory_id
            )
    ...
    fn get_mut_territories_with_occupation_ongoing(
        &mut self,
    ) -> Vec<(&TerritoryId, &mut Territory)> {
        self.territories
            .iter_mut()
            .filter(|(_, territory)| {
                territory
    ...
    pub fn occupation_checker(&mut self, delta: f64) {
        ...
        let territories_with_occupation_on_going =
            self.get_mut_territories_with_occupation_ongoing();
        for (territory_id, territory) in territories_with_occupation_on_going {
            let Some(ref player_static_info) = territory.player_trying_to_conquer else {
                godot_error!(
                    "'player_trying_to_conquer' doesn't exist during the Occupation Checker"
                );
                return;
            };
            ...
            territory.conquering_progress_per_second +=
                delta * (num_of_troops_in_the_territory as f64);
    ...
    fn check_territory_under_conflict(&mut self) {
        self.get_mut_territories_under_conflict()
            .iter_mut()
            .for_each(|(_, territory)| {
