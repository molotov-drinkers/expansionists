...
use godot::{
    builtin::Color,
    classes::{MeshInstance3D, StandardMaterial3D},
    prelude::*,
};
...
use crate::{
    globe::coordinates_system::surface_point::Coordinates, player::player::PlayerStaticInfo,
};
...
impl Territory {
    ...
    pub fn get_base_territory(
        territory_id: &str,
        continent: Continent,
        sub_continent: Option<SubContinent>,
    ) -> Territory {
        Territory {
            ...
            territory_states: HashSet::from([TerritoryState::Unoccupied]),
    ...
    fn continent_to_color(continent: &Continent) -> Color {
        match continent {
            ...
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Continent::Asia =>
            /* Color::GREEN_YELLOW */
            {
    ...
    pub fn get_territory_color(
        sub_continent: &Option<SubContinent>,
        continent: &Continent,
    ) -> Color {
        match sub_continent {
            Some(SubContinent::MiddleEast) =>
            /* Color::GREEN */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Some(SubContinent::InteriorAsia) =>
            /* Color::WEB_GREEN */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Some(SubContinent::IndianSubcontinent) =>
            /* Color::LAWN_GREEN */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Some(SubContinent::SoutheastAsia) =>
            /* Color::LIME_GREEN */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Some(SubContinent::EastAsia) =>
            /* Color::GREEN_YELLOW */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
            Some(SubContinent::EuropeRelatedAsia) =>
            /* Color::LIGHT_GREEN */
            {
                Color::ROSY_BROWN.lightened(0.7)
            }
    ...
    pub fn set_shade_color_to_territory(
        territory_mesh: Gd<MeshInstance3D>,
    ...
    pub fn set_troops_growth_velocity_and_secs_to_spawn(&mut self) {
        ...
        self.troops_growth_velocity =
            (Self::BASE_TROOP_GROWTH_VELOCITY * num_of_coordinates as f32).clamp(0.01, 3.);

        self.seconds_to_spawn_troop =
            Self::BASE_SECONDS_FOR_A_TROOP_TO_SPAWN / self.troops_growth_velocity as f64;
    }
    ...
    pub fn set_organic_max_troops(&mut self) {
        ...
        self.organic_max_troops = ((Self::BASE_TROOP_NUMBER_PER_TERRITORY
            * num_of_coordinates as f32) as u32)
    ...
    pub fn inform_troop_arrived(&mut self, troop_id: &TroopId, player_id: PlayerId) {
        self.all_troops_deployed_and_arrived
            .insert(troop_id.clone());
    ...
    pub fn inform_territory_departure(&mut self, troop_id: &TroopId, player_id: PlayerId) {
        ...
        if let Some(player_troops) = self
    ...
    pub fn set_troops_from_different_players_flag(&mut self) {
        ...
        self.all_troops_deployed_and_arrived_by_player
            ...
            .for_each(|(_, troops)| {
                if !troops.is_empty() {
                    troops_by_player_counter += 1
                }
            });

        self.has_troops_from_different_players = troops_by_player_counter > 1;
    }
}
