...
impl Land {
                ...
                match (mouse_button, pressed) {
                    ...
                    (MouseButton::RIGHT, false) => {
                        ...
                        if let Some(surface_point) = surface_point {
                            ...
                            let territory_id = surface_point_metadata
                                .territory_id
                                .clone()
                            ...
                            godot_error!(
                                "Err: clicked at {:?} and {:?} didn't find any surface point",
                                territory,
                                event_position
                            );
                        }
    ...
    fn get_root_from_land(&mut self) -> Gd<Node> {
        self.base()
            .get_parent()
            .expect("Expected Land to have mesh Territory as parent")
            .get_parent()
            .expect("Expected Mesh territory to have territories as parent")
            .get_parent()
            .expect("Expected territories to have globe as parent")
            .get_parent()
            .expect("Expected globe to have root as parent")
    }

    fn get_territory_hud_from_land(&mut self) -> Gd<TerritoryHUD> {
        self.get_root_from_land()
