        ...
        if self.current_territory.is_some() {
            ...
            if !self.language_set {
                ...
                self.chosen_language =
                    Player::get_player_language(self.get_root_from_territory_hud(), MAIN_PLAYER_ID);
...
impl TerritoryHUD {
    ...
    fn activate_territory_part(&mut self, territory: &Territory) {
        let shared_path =
            "territory_margin_container/PanelContainer/MarginContainer/VBoxContainer/";

        let mut name = self
            .base()
            .get_node_as::<TextLabels>(&(shared_path.to_owned() + "TextLabels"));
        let mut size_info = self
            .base()
            .get_node_as::<TextLabels>(&(shared_path.to_owned() + "size_info/TextLabels"));
        let mut continent = self
            .base()
            .get_node_as::<TextLabels>(&(shared_path.to_owned() + "continent/TextLabels"));
        ...
        let translated_sub_continent =
            if let Some(sub_continent) = &territory.location.sub_continent {
        ...
        let translated_every_x_secs =
            translated_every_x_secs.replace("{x}", formatted_secs_to_troop.as_str());

        name.set_text(&translated_territory.to_godot());
        name.bind_mut().set_font_size(32);

        size_info.set_text(&format!(
            "{translated_size} [{translated_every_x_secs} -> +{max_troops}]"
        ));
    ...
    fn activate_ruler_part(&mut self, territory: &Territory) {
        ...
        if territory
            .territory_states
            .contains(&TerritoryState::UnoccupiedUnderConflict)
        {
            ...
            .territory_states
            .contains(&TerritoryState::OccupationInProgress)
        {
            ...
            .territory_states
            .contains(&TerritoryState::Unoccupied)
        {
            ...
            .territory_states
            .contains(&TerritoryState::OccupiedUnderConflict)
        {
            ...
            .territory_states
            .contains(&TerritoryState::Occupied)
        {
    ...
    fn get_root_from_territory_hud(&mut self) -> Gd<RootScene> {
        self.base()
            .get_parent()
            .expect("Expected TerritoryHUD to have ui as parent")
            .get_parent()
            .expect("Expected ui to have root as parent")
            .cast::<RootScene>()
    }
    ...
    fn get_ruler_states_godot_classes(
        &mut self,
    ) -> (
        Gd<TextLabels>,
        Gd<HBoxContainer>,
        ...
        let ruler_label = self
            .base()
            .get_node_as::<TextLabels>(&(shared_path.to_owned() + "HBoxContainer/TextLabels"));

        let mut unoccupied = self
            .base()
            .get_node_as::<HBoxContainer>(&(shared_path.to_owned() + "unoccupied"));

        let mut under_conflict = self
            .base()
            .get_node_as::<VBoxContainer>(&(shared_path.to_owned() + "under_conflict"));

        let mut occupation_in_progress = self
            .base()
            .get_node_as::<HBoxContainer>(&(shared_path.to_owned() + "occupation_in_progress"));

        let mut occupied = self
            .base()
            .get_node_as::<HBoxContainer>(&(shared_path.to_owned() + "occupied"));
    ...
    fn show_updated_unoccupied_under_conflict_ruler_hud(
        under_conflict: &mut Gd<VBoxContainer>,
        ruler_label: &mut Gd<TextLabels>,
        general_dictionary: &I18nDefaultDictionary,
    ) {
    ...
    fn show_updated_occupation_in_progress_ruler_hud(
        occupation_in_progress: &mut Gd<HBoxContainer>,
        ruler_label: &mut Gd<TextLabels>,
        general_dictionary: &I18nDefaultDictionary,
        territory: &Territory,
    ) {
        ...
        let Some(player_trying_to_conquer) = &territory.player_trying_to_conquer else {
            return;
        };
        ...
        let Some(all_troops_deployed_and_arrived_by_occupier) = territory
        ...
        occupation_in_progress
            .get_node_as::<TextLabels>("VBoxContainer/troops/TextLabels")
            .set_text(&format!("{:?}x", num_of_troops));

        let mut occupation_progress_bar =
            occupation_in_progress.get_node_as::<ProgressBar>("HBoxContainer/ProgressBar");
        let mut occupation_progress_text =
            occupation_in_progress.get_node_as::<TextLabels>("HBoxContainer/TextLabels");

        let percentage =
            territory.conquering_progress_per_second * 100. / territory.time_to_be_conquered;
    ...
    fn show_updated_unoccupied_ruler_hud(
        unoccupied: &mut Gd<HBoxContainer>,
        ruler_label: &mut Gd<TextLabels>,
        general_dictionary: &I18nDefaultDictionary,
    ) {
    ...
    fn show_updated_occupied_ruler_hud(
        occupied: &mut Gd<HBoxContainer>,
        ruler_label: &mut Gd<TextLabels>,
        general_dictionary: &I18nDefaultDictionary,
        territory: &Territory,
    ) {
        ...
        let Some(all_troops_deployed_and_arrived_by_ruler) = territory
            .all_troops_deployed_and_arrived_by_player
            .get(&ruler.player_id)
        else {
            godot_warn!(
        ...
        let mut next_troop_progress_bar =
            occupied.get_node_as::<ProgressBar>("HBoxContainer/ProgressBar");
        let mut next_troop_progress_text =
            occupied.get_node_as::<TextLabels>("HBoxContainer/TextLabels");
    ...
    fn show_updated_occupied_under_conflict_ruler_hud(
        under_conflict: &mut Gd<VBoxContainer>,
        ruler_label: &mut Gd<TextLabels>,
        _general_dictionary: &I18nDefaultDictionary,
        territory: &Territory,
    ) {
        under_conflict.set_visible(true);
        ...
        let player_trying_to_conquer_color =
            PlayerColor::get_banner_player_color(&player_trying_to_conquer.color);
        let Some(all_troops_deployed_and_arrived_by_player_trying_to_conquer) = territory
            .all_troops_deployed_and_arrived_by_player
            .get(&player_trying_to_conquer_id)
        else {
            godot_warn!(
                "Expected to find player_id in all_troops_deployed_and_arrived_by_player\n
        TerritoryHUD::show_updated_occupation_in_progress_ruler_hud"
        ...
        let num_of_troops_of_player_trying_to_conquer =
            all_troops_deployed_and_arrived_by_player_trying_to_conquer.len() as f32;

        let Some(ruler) = &territory.current_ruler else {
            return;
        };
        ...
        let right_ratio =
            num_of_troops_of_player_trying_to_conquer * MAX_STRAIGHT_RATIO / total_troops;

        let mut fire_power_left_bar =
            under_conflict.get_node_as::<ColorRect>("fire_power_bars/left_bar");
        let mut fire_power_right_bar =
            under_conflict.get_node_as::<ColorRect>("fire_power_bars/right_bar");
        ...
        let mut banner_right =
            under_conflict.get_node_as::<ColorRect>("HBoxContainer/banner_right");
        banner_left.set_color(ruler_color);
        banner_right.set_color(player_trying_to_conquer_color);

        under_conflict
            .get_node_as::<TextLabels>("HBoxContainer/troops_left/TextLabels")
            .set_text(&format!("{:.0}x", num_of_troops_of_ruler));
        under_conflict
            .get_node_as::<TextLabels>("HBoxContainer/troops_right/TextLabels")
            .set_text(&format!(
                "{:.0}x",
                num_of_troops_of_player_trying_to_conquer
            ));

        ruler_label.set_text(&format!(
            "{:} x {:}",
            &ruler.user_name.as_str(),
            &player_trying_to_conquer.user_name.as_str()
        ));
    }
}
