use crate::{
    ...
    globe::territories::{land::Land, territory::TerritoryId},
    heads_up_display::selection_hud::SelectionHUD,
    troops::troop::Troop,
};
use godot::{
    classes::{INinePatchRect, InputEvent, InputEventMouseButton, NinePatchRect},
    global::MouseButton,
    prelude::*,
};
...
impl UiDragBox {
    ...
    fn cast_troop_selection(&mut self) {
        ...
        for troop in self.get_player_troops().iter() {
            ...
            if !player_camera
                .bind_mut()
                .is_body_visible_on_camera(troop_position)
            {
                continue;
            }

            let troop_2d_position = player_camera
                .bind_mut()
            ...
            if in_the_rect {
                ...
                selection_hud
                    .bind_mut()
                    .set_text_with_num_of_troops(self.in_rect_troops.len());
            }
        }
    }

    fn deselect_troops(&mut self) {
        self.get_player_troops().iter().for_each(|troop| {
            let mut troop = troop.clone();
    ...
    fn set_reception_for_right_click_on_lands_signal(&mut self) {
        ...
        for land in all_territory_lands.iter_shared() {
            ...
            let callable = self.base_mut().callable("move_selected_troops");
    ...
    fn move_selected_troops(&mut self, moving_to: Vector3, territory_id: TerritoryId) {
        self.in_rect_troops.iter_mut().for_each(|troop| {
            troop
                .bind_mut()
                .set_order_to_move_to(moving_to, &territory_id);
        });
    }
    ...
    fn get_root_from_ui_drag_box(&mut self) -> Gd<Node> {
        self.base()
            .get_parent()
            .expect("Expected UiDragBox to have SelectionSystem as parent")
            .get_parent()
            .expect("Expected SelectionSystem to have playable as parent")
            .get_parent()
            .expect("Expected playable to have root as parent")
    }

    fn get_camera_from_ui_drag_box(&mut self) -> Gd<PlayerCamera> {
        self.get_root_from_ui_drag_box()
    ...
    fn get_ui_drag_box_rect(&mut self) -> Rect2 {
        let ui_drag_box_rect_position = match (self.positive_x, self.positive_y) {
            ...
            (false, true) => {
                Vector2::new(self.released_at.x, self.base_mut().get_rect().position.y)
            }
        };
